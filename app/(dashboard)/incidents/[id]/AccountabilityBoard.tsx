...
import {
  initIncidentLanes, addIncidentLane,
...
export default function AccountabilityBoard({
  ...
  // ── Init lanes from template ──────────────────────────────────────────────
  async function handleInit() {
    setError(null)
    const res = await initIncidentLanes(incidentId)
  ...
  async function handleAddLane() {
    ...
    setSavingLane(true)
    const res = await addIncidentLane(incidentId, newLaneName.trim())
    setSavingLane(false)
    if (res?.error) { setError(res.error); return }
  ...
  // ── Resolve scanned card to personnel ────────────────────────────────────
  ...
  async function handleScan(raw: string) {
    ...
    const res = await checkInPerson(incidentId, laneId, resolved.personnelId, resolved.rawName, resolved.rawDept)
    if (res?.error) { setError(res.error); return }
    if (res.entry) {
  ...
  async function handleManualAdd() {
    ...
    const res = await checkInPerson(incidentId, laneId, personnelId, rawName, rawDept)
  ...
  async function handlePAR() {
    ...
    const res = await recordPAR(incidentId, snapshot)
    setParSaving(false)
    if (res?.error) { setError(res.error); return }
    setParDone(true)
  ...
  if (lanes.length === 0) {
    return (
      <div className="text-center py-8">
        <p className="text-sm text-zinc-500 mb-4">Accountability not started for this incident.</p>
        {isOfficerOrAbove && (
