...
import { savePermitOfficerSignature } from '@/app/actions/public-site'

export default function PermitSignatureModal({
  permitId,
  contactName,
  confirmationCode,
  onClose,
  ...
  function handleConfirm() {
    ...
    canvasRef.current!.toBlob(blob => {
      ...
      startTransition(async () => {
        const result = await savePermitOfficerSignature(formData)
        if (result?.error) { setError(result.error); return }
        onSaved(result.signedAt!)
        onClose()
      })
    }, 'image/png')
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-white">
      ...
      <div className="shrink-0 px-5 py-4 border-b border-zinc-200 bg-zinc-50">
        <p className="text-xs font-semibold uppercase tracking-wide text-zinc-400">Officer Signature — Burn Permit</p>
        <p className="text-xl font-bold text-zinc-900 mt-0.5">{contactName}</p>
        <p className="text-sm text-zinc-500">Permit {confirmationCode}</p>
      </div>
      ...
      <div className="shrink-0 border-t border-zinc-200 bg-zinc-50 px-5 py-4 flex items-center gap-3">
        ...
        <button onClick={handleClear} disabled={isPending || isEmpty}
          className="rounded-lg border border-zinc-300 px-4 py-2 text-sm font-semibold text-zinc-600 hover:bg-zinc-100 transition-colors disabled:opacity-50">
          Clear
        </button>
        {error && <p className="flex-1 text-center text-sm text-red-600">{error}</p>}
        <button onClick={handleConfirm} disabled={isPending || isEmpty}
