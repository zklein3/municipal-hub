...
import { saveCertSignature } from '@/app/actions/training'

export default function CertSignaturePadModal({
  memberName,
  certName,
  certId,
  personnelId,
  ...
  function handleClear() { padRef.current?.clear(); setIsEmpty(true); setError(null) }
  ...
  function handleConfirm() {
    ...
    canvasRef.current!.toBlob(blob => {
      ...
      startTransition(async () => {
        const result = await saveCertSignature(formData)
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-white">
      <div className="shrink-0 px-5 py-4 border-b border-zinc-200 bg-zinc-50">
        ...
        <p className="text-sm text-zinc-500">{certName}</p>
      </div>
