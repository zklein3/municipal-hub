...
- **Casualty module** (`casualty_rescues[]`) — same "our own readiness check, not NERIS" gap found and fixed 2026-09-22 for the fire side: `modules.casualty` triggers on every fire-coded incident (garage fire, outbuilding fire, etc.), but most fires have nobody hurt. `buildNerisPayload` already guarded `casualty_rescues` with `if (rescuePersons.length > 0)`, so an empty module was already being dropped correctly, same as `medical_details` above — the block was purely our `casualty.victims` requirement in `lib/neris-requirements.ts`. Added `incident_neris.no_victims` (migration `add_incident_neris_no_victims`, same shape as `no_patient_contact`: boolean, not null, default false) — a checkbox in the Casualty — Victims section shown only while the victim list is empty, mirroring the Medical section's pattern exactly. Unchecked still blocks.
